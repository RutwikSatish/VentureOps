...
with st.sidebar:
    ...
    startup_name    = st.text_input("Startup Name", "Acme AI")
    ...
    team_size       = st.slider("Team Size (FTEs)", 1, 200, 12)
    founded_years   = st.slider("Years Since Founded", 0, 10, 2)
    ...
    monthly_revenue = st.number_input("Monthly Revenue ($)", 0, 10_000_000, 50_000, step=5_000)
    monthly_burn    = st.number_input("Monthly Gross Burn ($)", 1_000, 10_000_000, 150_000, step=5_000)
    cash_on_hand    = st.number_input("Cash on Hand ($)", 0, 100_000_000, 1_200_000, step=50_000)
    arpu            = st.number_input("Avg Revenue Per User/Month ($)", 1, 100_000, 299)
    cac             = st.number_input("Customer Acquisition Cost ($)", 1, 500_000, 900)
    gross_margin_pct= st.slider("Gross Margin (%)", 0, 100, 72)
    monthly_churn   = st.slider("Monthly Churn Rate (%)", 0.0, 20.0, 2.5, 0.1)
    mrr_growth      = st.slider("MoM Revenue Growth (%)", -20.0, 50.0, 8.0, 0.5)
...
with tab1:
    ...
    with col1:
        ...
        sc_single_source  = st.slider("Single-source supplier concentration", 0, 10, 5,
        ...
        sc_inventory      = st.slider("Inventory / delivery buffer adequacy", 0, 10, 5,
            help="10 = >60 days safety stock")

        st.markdown("#### Unit Economics Health")
        ue_ltv_cac        = st.slider("LTV:CAC ratio awareness", 0, 10, 5,
            help="0 = Unknown, 10 = Validated >3:1 with real cohort data")
        ue_payback        = st.slider("CAC payback period control", 0, 10, 5,
            help="10 = <12 months payback, 0 = >36 months")
        ue_margin         = st.slider("Gross margin sustainability", 0, 10, 6,
            help="10 = >70% and improving, 0 = Negative")

    with col2:
        st.markdown("#### Operational Leverage")
        ol_automation     = st.slider("Process automation maturity", 0, 10, 4,
            help="10 = Core ops fully automated, 0 = All manual")
        ol_scalability    = st.slider("Revenue scalability without headcount", 0, 10, 5,
            help="10 = Near-zero marginal cost to serve new customers")

        st.markdown("#### Team Capacity & Execution")
        tc_key_person     = st.slider("Key-person dependency risk", 0, 10, 6,
            help="10 = Critical functions concentrated in 1-2 people")
        tc_hiring         = st.slider("Hiring pipeline for critical roles", 0, 10, 5,
            help="10 = Active pipeline, documented processes")

        st.markdown("#### Process Maturity")
        pm_documentation  = st.slider("SOPs and process documentation", 0, 10, 4,
            help="10 = All core processes documented and followed")
        pm_kpis           = st.slider("KPI framework and monitoring", 0, 10, 5,
            help="10 = Real-time dashboards, weekly review cadence")

        st.markdown("#### Tech Debt Exposure")
        td_infrastructure = st.slider("Infrastructure scalability", 0, 10, 5,
            help="10 = Cloud-native, auto-scaling, 0 = Single server")
        td_debt_level     = st.slider("Known tech debt severity", 0, 10, 4,
            help="10 = Critical unresolved debt in core systems")

    # ── SCORING ──────────────────────────────────────────────────────────────────
    # Supply chain: invert single_source and multiply, lower = higher risk
    ...
    # Weights based on startup failure analysis (CB Insights, Bessemer)
    weights = {
    ...
    📚 <b>Scoring Framework:</b> Dimension weights derived from CB Insights "Why Startups Fail" analysis (n=111), 
    Bessemer Venture Partners State of the Cloud benchmarks, and McKinsey operational due diligence framework for 
    PE/VC transactions. Unit economics weighted highest (0.22) as the #1 predictor of post-Series A survival.
    </div>
    """, unsafe_allow_html=True)
...
with tab2:
    ...
    with col1:
        ...
        st.markdown("*Start with your actual customer and pricing data*")

        total_potential_customers = st.number_input("Total potential customers in your market", 100, 100_000_000, 50_000, step=1000)
        acv_annual                = st.number_input("Annual Contract Value / Revenue per customer ($)", 10, 1_000_000, 3_600, step=100)
        sam_filter_pct            = st.slider("% of TAM you can realistically serve today\n(geography, product fit, channels)", 1, 100, 25)
        som_capture_pct           = st.slider("% of SAM you can capture in 3 years\n(competition, sales capacity, brand)", 1, 50, 8)
    ...
    with col2:
        ...
        st.markdown("*Start from industry reports, narrow down*")

        industry_market_size = st.number_input("Total industry market size ($B from reports)", 0.1, 5000.0, 12.5, step=0.5)
        td_segment_pct       = st.slider("% of industry relevant to your segment", 1, 100, 18)
        td_geo_pct           = st.slider("% addressable by your geographic reach", 1, 100, 35)
        td_product_fit_pct   = st.slider("% matching your product's current features", 1, 100, 60)
    ...
    with c4:
        ...
        st.metric("Primary SOM",     fmt_bn(som_bu), help="Bottom-up SOM is your primary number for investor conversations")
    ...
    SOM uses realistic penetration rate anchored in sales capacity, not % of TAM (avoids the "1% fallacy" 
    flagged by Pear VC). Cross-check: if top-down and bottom-up diverge by >2x, revisit assumptions 
    (Data-Mania, 2026).
    </div>
    """, unsafe_allow_html=True)
...
with tab3:
    ...
    annual_growth   = (mrr_growth / 100) * 12 * 100  # rough annualized
    ...
    def simulate_runway(rev_growth, burn_growth, months=36):
        ...
        for m in range(1, months + 1):
            ...
            rev_hist.append(revenue)
            if cash <= 0 and zero_month is None:
                zero_month = m
        return cash_hist, rev_hist, zero_month

    months_range = list(range(37))
    cons_cash, cons_rev, cons_zero = simulate_runway(cons_rev_growth, cons_burn_growth)
    base_cash, base_rev, base_zero = simulate_runway(base_rev_growth, base_burn_growth)
    opt_cash,  opt_rev,  opt_zero  = simulate_runway(opt_rev_growth,  opt_burn_growth)
    ...
    Burn multiple benchmarks from Bessemer Venture Partners (< 1 = Excellent, 1–1.5 = Good, 1.5–2 = OK, 
    2–3 = Suspect, >3 = Poor). Rule of 40 per Bain & Company definition. Fundraising window: 6-9 months 
    before runway exhaustion (standard VC advice). 38-44% of startups fail from running out of cash 
    (CB Insights, 2024).
    </div>
    """, unsafe_allow_html=True)
...
with tab4:
    ...
    st.markdown("Stress-tests the startup at 2x, 5x, and 10x current scale across 5 operational vectors. "
                "Predicts what breaks first and when.")

    col1, col2 = st.columns(2)

    with col1:
        st.markdown("#### Current State Assessment")
        current_customers    = st.number_input("Current active customers", 1, 1_000_000, 150)
        ops_team_size        = st.slider("Ops/CS team size (FTEs)", 1, 100, 3)
        customers_per_csm    = st.slider("Customers each ops/CS person handles", 1, 500, 50)
        infra_cost_per_cust  = st.number_input("Monthly infra cost per customer ($)", 0.01, 1000.0, 8.5)
        supplier_count       = st.slider("Number of active suppliers / vendors", 1, 200, 4)
        single_source_pct    = st.slider("% of revenue dependent on top supplier", 0, 100, 65)

    with col2:
        st.markdown("#### Process & Tech Inputs")
        manual_ops_hrs_week  = st.slider("Manual ops hours per week", 0, 200, 35)
        tech_incidents_month = st.slider("Tech incidents / outages per month", 0, 50, 3)
        hiring_time_weeks    = st.slider("Average weeks to hire a key role", 1, 52, 10)
        monthly_hiring_budget= st.number_input("Monthly hiring budget headroom ($)", 0, 500_000, 25_000)
    ...
    def score_vector(name, values, thresholds_bad, thresholds_warn):
        """Returns risk score 0-10 for each scale point"""
        scores = []
        for v in values:
            if v >= thresholds_bad:
                scores.append(8.5)
            elif v >= thresholds_warn:
                scores.append(5.5)
            else:
                scores.append(2.5)
        return scores

    # 5 bottleneck vectors
    headcount_needed   = [current_customers * s / customers_per_csm for s in scales]
    headcount_risk     = [h / ops_team_size for h in headcount_needed]  # ratio
    ...
    cashflow_pressure  = [(net_burn * s / monthly_revenue) if monthly_revenue > 0 else s for s in scales]
    supplier_risk      = [single_source_pct * (1 + (s - 1) * 0.15) for s in scales]  # concentration worsens at scale

    # Risk scoring per vector per scale
    vectors = {
    ...
    fig_heat = px.imshow(
        ...
        zmin=0, zmax=10,
        title="Bottleneck Risk Heatmap (0 = Low, 10 = Critical)",
    ...
    st.markdown("#### 🚨 Predicted First Bottleneck")
    ...
    with col1:
        ...
        <b>At 2x Scale:</b> {worst_2x[0]} is your biggest risk (score: {worst_2x[1]:.1f}/10)
        </div>
        """, unsafe_allow_html=True)
    with col2:
        st.markdown(f"""
        <div class="danger-box">
        <b>At 5x Scale:</b> {worst_5x[0]} becomes critical (score: {worst_5x[1]:.1f}/10)
        </div>
        """, unsafe_allow_html=True)
    ...
    interventions = {
        "👥 Hiring Pressure":       "Build a talent pipeline now. Document all roles. Use structured hiring. Each delay in a key hire costs 6-8 weeks at scale.",
        "💻 Infra Cost Burden":     "Audit cloud architecture. Implement auto-scaling. Negotiate volume commitments. Target <10% of revenue on infra by Series A.",
        "⚙️ Manual Ops Overload":   "Automate top 3 repetitive workflows before 2x. Manual processes that take 35 hrs/week will take 175 hrs at 5x — unsustainable.",
        "💰 Cash Flow Stress":      "Extend runway before aggressive growth. Start Series A process when 7+ months remain. Model all 3 burn scenarios monthly.",
        "🔗 Supplier Concentration":"Qualify 1-2 backup suppliers now. Single-source concentration above 60% is an existential risk at scale.",
    }
    ...
    st.markdown("""
    <div class="framework-note">
    📚 <b>Framework:</b> Scale bottleneck analysis derived from Blitzscaling (Reid Hoffman), 
    The Lean Startup (Eric Ries) constraint theory, and Sequoia Capital's operational maturity rubric. 
    Supplier concentration thresholds based on supply chain resilience research (MIT CTL). 
    Hiring pressure model based on 1 CS/ops FTE per 50 SMB customers benchmark (SaaS Capital).
    </div>
    """, unsafe_allow_html=True)
...
with tab5:
    ...
    st.markdown("Powered by **Groq (Llama 3)**. Generates a structured advisor-grade brief based on all inputs above.")

    if st.button("⚡ Generate AI Strategic Brief", type="primary", use_container_width=True):
        # Assemble context from all modules
        context = f"""
You are a senior startup advisor at the Roux Institute, helping founders and investors 
through operational due diligence. You have just completed a full analysis of a startup.
...
- Years Founded: {founded_years}

FINANCIAL HEALTH:
- Monthly Revenue: ${monthly_revenue:,.0f}
- Monthly Gross Burn: ${monthly_burn:,.0f}
- Net Burn: ${net_burn:,.0f}/month
- Cash on Hand: ${cash_on_hand:,.0f}
- Runway: {runway_months:.0f} months
- MoM Revenue Growth: {mrr_growth}%
...
- CAC: ${cac:,.0f}
- LTV: ${ltv:,.0f}
- LTV:CAC Ratio: {ltv_cac:.1f}x (target >3x)
- CAC Payback: {payback_months:.0f} months (target <18)
- Burn Multiple: {burn_multiple:.2f}x (target <1.5)
- Rule of 40 Score: {rule_of_40:.0f} (target ≥40)

MARKET SIZING:
- TAM (Bottom-Up): {fmt_bn(tam_bu)}
- SAM: {fmt_bn(sam_bu)}
- SOM (3-year target): {fmt_bn(som_bu)}

OPERATIONAL READINESS:
- Overall Score: {total_score:.1f}/10
- Classification: {classification}
- Weakest Dimension: {weakest[0]} ({weakest[1][0]:.1f}/10)
- Strongest Dimension: {max(weights.items(), key=lambda x: x[1][0])[0]}

SCALE BOTTLENECK (5x stress test):
- Biggest risk at 5x: {worst_5x[0]} (score {worst_5x[1]:.1f}/10)
- Biggest risk at 2x: {worst_2x[0]} (score {worst_2x[1]:.1f}/10)

Produce a structured strategic brief with exactly this format:

## SITUATION SUMMARY
[2-3 sentences on overall startup health and stage-appropriate assessment]

## TOP 3 RISKS
Risk 1 — [Name]: [Specific, data-backed description with the number from the analysis]
Risk 2 — [Name]: [Specific, data-backed description with the number from the analysis]
Risk 3 — [Name]: [Specific, data-backed description with the number from the analysis]

## TOP 3 OPPORTUNITIES
Opportunity 1 — [Name]: [Specific opportunity with supporting rationale]
Opportunity 2 — [Name]: [Specific opportunity with supporting rationale]
Opportunity 3 — [Name]: [Specific opportunity with supporting rationale]

## 90-DAY INTERVENTION PLAN
Week 1-2: [Most urgent action]
Week 3-4: [Second priority]
Month 2: [Third priority]
Month 3: [Fourth priority]

## THE SINGLE METRIC TO WATCH
[One metric, why it matters most right now, and what the threshold for action is]

## INVESTOR READINESS VERDICT
[One paragraph: Is this startup ready to raise? What needs to change first?]
"""
        ...
        try:
            ...
            st.error("⚠️ GROQ_API_KEY not found in Streamlit secrets. "
                     "Add it to `.streamlit/secrets.toml` as: `GROQ_API_KEY = 'your_key'`")
    ...
    for item in [
        ...
        "Investor readiness verdict — honest and direct",
    ]:
        st.markdown(f"- {item}")

    st.markdown("""
    <div class="framework-note">
    📚 <b>AI Layer:</b> Uses Groq's Llama 3 70B model. Brief structure adapted from Y Combinator 
    partner review format, Sequoia's Arc framework for company health, and First Round Capital's 
    operational assessment rubric. All context injected from your live inputs — no generic responses.
    </div>
    """, unsafe_allow_html=True)
...
Frameworks: CB Insights · Bessemer VP · McKinsey · Antler · Blitzscaling (Hoffman) · 
The Lean Startup (Ries) · Y Combinator · Sequoia Capital<br>
